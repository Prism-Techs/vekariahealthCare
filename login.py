...
"            }")
        ...
        self.username.focusInEvent = lambda event: self.show_keyboard(event, self.username)
        self.username.focusOutEvent = lambda event: self.hide_keyboard(event)
        font = QtGui.QFont()
        font.setPointSize(18)
...
"            }")
        ...
        self.password.focusInEvent = lambda event: self.show_keyboard(event, self.password)
        self.password.focusOutEvent = lambda event: self.hide_keyboard(event)
...
"}")
    ...
    def show_keyboard(self, event, widget):
        """Show the on-screen keyboard when a text field gains focus"""
        try:
            # Kill any existing keyboard instance
            subprocess.run(['killall', 'onboard'], stderr=subprocess.DEVNULL)
            
            # Set environment variable for keyboard position
            os.environ['ONBOARD_WINDOW_STATE_STICKY'] = '1'
            
            # Launch onboard keyboard at bottom of screen
            subprocess.Popen(['onboard', '--xid', '--layout', 'Full', 
                            '--size=1024x300', '--x=0', '--y=300'])
            
            # Accept the focus event
            widget.focusInEvent = lambda e: QtWidgets.QLineEdit.focusInEvent(widget, e)
            widget.focusInEvent(event)
        except Exception as e:
            print(f"Error showing keyboard: {e}")

    def hide_keyboard(self, event):
        """Hide the on-screen keyboard when focus is lost"""
        try:
            subprocess.run(['killall', 'onboard'], stderr=subprocess.DEVNULL)
        except Exception as e:
